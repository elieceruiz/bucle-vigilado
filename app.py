...
def calcular_racha_detallada(eventos, ahora):
    if not eventos:
        return "0 minutos", "0 años, 0 meses, 0 días, 0 horas, 0 min, 0 s"
    ultimo = eventos[0]["fecha_hora"].astimezone(colombia)
    delta = ahora - ultimo
    total_minutos = int(delta.total_seconds() // 60)
    rdelta = relativedelta(ahora, ultimo)
    tiempo_detallado = f"{rdelta.years} años, {rdelta.months} meses, {rdelta.days} días, {rdelta.hours} horas, {rdelta.minutes} min, {rdelta.seconds} s"
