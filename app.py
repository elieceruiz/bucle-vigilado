...
def clasificar_reflexion_openai(texto):
    prompt = f"""Sistema categorial para clasificar reflexiones. Responde solo con el código correspondiente (ej: 1.1, 2.2):
Reflexión: \"\"\"{texto}\"\"\""""
...
def obtener_reflexiones():
    ...
    for r in registros:
        ...
        codigo_cat = r.get("categoria_categorial", "")
...
def mostrar_racha(nombre_evento, emoji):
    ...
    st.metric("Duración", f"{int(delta.total_seconds()//60)} min", formatear_delta(d, incluir_segundos=True))
    ...
    with tabs[0]:
        ...
        for _, r in df.iterrows():
            with st.expander(f"{r['Fecha']} {r['Hora']} {r['Emociones']}"):
                ...
                st.caption(f"Categoría: {r['Categoría']} | Subcategoría: {r['Subcategoría']}")
