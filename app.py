...
# Al iniciar, carga último evento registrado para eventos principales a session_state
...
# Validaciones para eventos principales solo si se ve la racha (checkbox activo)
...
# Limpieza de estados temporales para reflexiones si seleccionan otra opción
if opcion != "reflexion":
    for key in ["texto_reflexion", "emociones_reflexion", "reset_reflexion"]:
        if key in st.session_state:
            del st.session_state[key]

# Módulo registrar eventos y mostrar racha
if opcion in [evento_a, evento_b]:
...
# Módulo historial
    ...
    with tabs[1]:
        df_evento_a = obtener_registros(evento_a)
        mostrar_tabla_a = st.session_state.get("mostrar_completo_evento_a", False)
        if mostrar_tabla_a:
            st.markdown(f"**Total de registros:** {len(df_evento_a)}")
            st.dataframe(df_evento_a, use_container_width=True, hide_index=True)
        else:
            st.markdown("**Total de registros:** 🔒")
            st.dataframe(pd.DataFrame(columns=df_evento_a.columns), use_container_width=True, hide_index=True)

    with tabs[2]:
        df_evento_b = obtener_registros(evento_b)
        mostrar_tabla_b = st.session_state.get("mostrar_completo_evento_b", False)
        if mostrar_tabla_b:
            st.markdown(f"**Total de registros:** {len(df_evento_b)}")
            st.dataframe(df_evento_b, use_container_width=True, hide_index=True)
        else:
            st.markdown("**Total de registros:** 🔒")
            st.dataframe(pd.DataFrame(columns=df_evento_b.columns), use_container_width=True, hide_index=True)
